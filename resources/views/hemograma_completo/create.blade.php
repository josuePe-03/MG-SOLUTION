<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-semibold text-gray-800">Crear Hemograma Completo</h2>
    </x-slot>

    <div class="max-w-xl mx-auto mt-6 bg-white rounded-lg shadow p-6">
        <form action="{{ route('hemograma_completo.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-3">
                ...
                <select name="idCategoriaHemogramaCompleto" class="w-full border px-3 py-2 rounded focus:outline-none focus:ring" required>
                    ...
                    @foreach($categorias as $c)
                        <option value="{{ $c->id }}" {{ old('idCategoriaHemogramaCompleto') == $c->id ? 'selected' : '' }}>{{ $c->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="idUnidad" class="w-full border px-3 py-2 rounded focus:outline-none focus:ring" required>
                    ...
                    @foreach($unidades as $u)
                        <option value="{{ $u->id }}" {{ old('idUnidad') == $u->id ? 'selected' : '' }}>{{ $u->nombre }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="referencia" value="{{ old('referencia') }}" class="w-full border px-3 py-2 rounded focus:outline-none focus:ring" required>
